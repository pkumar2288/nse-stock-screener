...
# ============= CONFIGURATION =============
# Detect run mode - CLOUD mode for GitHub Actions, LOCAL for desktop
RUN_MODE = os.environ.get("RUN_MODE", "local")  # Set to "cloud" in GitHub workflow
SAVE_LOCAL_FILES = (RUN_MODE == "local")  # Only save files when running locally

# Telegram — priority: environment variables > hardcoded (for backward compatibility)
BOT_TOKEN = os.environ.get("TELEGRAM_BOT_TOKEN", "")
CHAT_ID = os.environ.get("TELEGRAM_CHAT_ID", "")

# Fallback to hardcoded values ONLY if environment variables are not set (for local testing)
if not BOT_TOKEN:
    BOT_TOKEN = "8736775563:AAH7jG4aHyUUfmtKJUxs-ChSXvOHp3Tm7Wk"  # Hardcoded token
if not CHAT_ID:
    CHAT_ID = "545110825"  # Hardcoded chat ID
...
# Batch processing - reduce for cloud to avoid timeouts
if RUN_MODE == "cloud":
...
def _write_excel(filename: str, sheet_name: str, df: pd.DataFrame) -> None:
    """Write a DataFrame to an Excel file with auto-fitted columns."""
    with pd.ExcelWriter(filename, engine="openpyxl") as writer:
        clean_name = sanitize_sheet_name(sheet_name)
        df.to_excel(writer, sheet_name=clean_name, index=False)
        autofit_worksheet_columns(writer.sheets[clean_name])
    log.info("Created: %s", filename)


def _write_excel_to_bytes(sheet_name: str, df: pd.DataFrame) -> bytes:
    """Write a DataFrame to Excel bytes (for cloud mode)."""
    output = BytesIO()
...
def create_excel_files(
    ...
    both_matches: set,
    timestamp: str,
) -> Tuple[Optional[str], Optional[str], Optional[str], Optional[bytes], Optional[bytes], Optional[bytes]]:
    """
    Create Excel files either on disk or in memory based on RUN_MODE.
    Returns: (odd_file_path, level_file_path, both_file_path, odd_bytes, level_bytes, both_bytes)
    """
    ...
    if odd_matches:
        ...
        df = pd.DataFrame(rows).sort_values("Deviation %")
        ...
        if SAVE_LOCAL_FILES:
            ...
            log.info(f"Created odd square Excel in memory ({len(odd_bytes)} bytes)")
    else:
        log.warning("No odd-square matches found")
    ...
    if level_matches:
        ...
        df = pd.DataFrame(rows).sort_values("Nearest Level Deviation %")
        ...
        if SAVE_LOCAL_FILES:
            ...
            log.info(f"Created level matches Excel in memory ({len(level_bytes)} bytes)")
    else:
        log.warning("No volatility-level matches found")
    ...
    if both_matches:
        ...
        df = pd.DataFrame(rows).sort_values(["Signal", "Strength"],
        ...
        if SAVE_LOCAL_FILES:
            ...
            log.info(f"Created both criteria Excel in memory ({len(both_bytes)} bytes)")
    else:
        log.warning("No both-criteria matches found")
...
def send_telegram_file_bytes(file_bytes: bytes, filename: str, caption: str = "") -> bool:
    """Upload a file from bytes to Telegram (for cloud mode)."""
    if not BOT_TOKEN or not CHAT_ID:
        log.debug("Telegram not configured - skipping")
        return False
...
def send_telegram_summary(
    ...
    level_matches: Dict,
    both_matches: set,
    odd_file: Optional[str],
    level_file: Optional[str],
    both_file: Optional[str],
    odd_bytes: Optional[bytes] = None,
    level_bytes: Optional[bytes] = None,
    both_bytes: Optional[bytes] = None,
) -> None:
    """Send the signal summary message then upload all three Excel files."""
    if not BOT_TOKEN or not CHAT_ID:
        log.debug("Telegram not configured - skipping summary")
        return

    now = datetime.now().strftime("%d %b %Y %H:%M")
    mode_str = "☁️ CLOUD" if not SAVE_LOCAL_FILES else "💻 LOCAL"
    ...
    if both_matches:
        ...
        for sym in sorted(both_matches):
            od = odd_matches[sym]
            ld = level_matches[sym]
    ...
    # Small delay to ensure message arrives first
    time.sleep(2)

    # ── 2. Upload each Excel file ────────────────────────────────
    if not SAVE_LOCAL_FILES:
        # Cloud mode: send from bytes
        if odd_bytes:
            send_telegram_file_bytes(odd_bytes, f"odd_square_matches_{now.replace(' ', '_').replace(':', '')}.xlsx",
                                    f"Odd Square matches ({len(odd_matches)} stocks)  |  {now}")
        if level_bytes:
            send_telegram_file_bytes(level_bytes, f"volatility_level_matches_{now.replace(' ', '_').replace(':', '')}.xlsx",
                                    f"Volatility Level matches ({len(level_matches)} stocks)  |  {now}")
        if both_bytes:
            send_telegram_file_bytes(both_bytes, f"both_criteria_signals_{now.replace(' ', '_').replace(':', '')}.xlsx",
                                    f"Both-criteria signals ({len(both_matches)} stocks)  |  {now}")
    else:
        # Local mode: send from files
        file_captions = [
            (odd_file,   f"Odd Square matches ({len(odd_matches)} stocks)  |  {now}"),
            (level_file, f"Volatility Level matches ({len(level_matches)} stocks)  |  {now}"),
            (both_file,  f"Both-criteria signals ({len(both_matches)} stocks)  |  {now}"),
        ]
        for filepath, caption in file_captions:
            if filepath:
...
def main(dry_run: bool = False, override_symbols: Optional[List[str]] = None) -> None:
    ...
    print("Odd Square +/-0.5%  |  Volatility Levels +/-1%  |  Wilder RSI-14 Daily + Weekly")
    print("=" * 80)
    ...
    odd_file, level_file, both_file, odd_bytes, level_bytes, both_bytes = create_excel_files(
        odd_matches, level_matches, both_matches, timestamp
    )
